...
URipple.prototype.init = function () {
  ...
  if (this._element) {
    ...
    this._ignoringMouseDown = false;
    on(this._element, 'mousedown', function (e) {
      self._down(e);
    });
    on(this._element, 'touchstart', function (e) {
      self._down(e);
    });

    on(this._element, 'mouseup', function (e) {
      self._up(e);
    });
    on(this._element, 'mouseleave', function (e) {
      self._up(e);
    });
    on(this._element, 'touchend', function (e) {
      self._up(e);
    });
    on(this._element, 'blur', function (e) {
      self._up(e);
    });
  }
};
 ...
 */
var on$1 = function on(name, _callback, one) {
    var self = this,
        origCb = _callback;
 ...
 */
var off$1 = function off(name, callback) {
    name = name.toLowerCase();
 ...
 */
var trigger$1 = function trigger(name) {
    name = name.toLowerCase();
...
DateTimePicker.fn._carousel = function (newPage, direction) {
    ...
    if (env.isIE8 || env.isIE9 || env.isFF || true) {
        // 动画存在问题，禁用动画
...
DateTimePicker.fn._fillDate = function (type) {
    ...
    on(dateDiv, 'click', function (e) {
        ...
        if (_cell) {
            ...
            if (env.isIE8 || env.isIE9) _cell.style.backgroundColor = "#fff";
        }
        addClass(e.target, 'current');
        if (env.isIE8 || env.isIE9) e.target.style.backgroundColor = '#3f51b5';
        this._updateDate();
        if (this.type === 'date') {
            this.onOk();
        }
...
DateTimePicker.fn._response = function () {
    ...
    var _height = 430;
    if (this.type === 'date' && !env.isMobile) _height = 395;
...
var ClockPicker = u.BaseComponent.extend({
    ...
    createPanel: function createPanel() {
        ...
        if (!env.isMobile) this.btnClean.style.display = 'none';
        this.currentView = 'hours';
        on(this.hourDiv, 'click', function (e) {
            var target = e.target;
            if (hasClass(target, 'clockpicker-tick')) {
