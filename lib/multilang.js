...
+function ($) {
	var Multilang = $.InputComp.extend({
		initialize: function initialize(element, options, viewModel) {
			var self = this;
		...
		//往模型上设置值
		setModelValue: function setModelValue(valObj) {
		...
		modelValuelangChange: function modelValuelangChange(value) {},
		Statics: {
			compName: 'multilang'
		}
	});

	if ($.compManager) $.compManager.addPlug(Multilang);
}($);